import React, { useState, useEffect } from 'react'; import { Zap, TrendingDown, ArrowRight, Play, Sparkles, Target, ShoppingCart, Factory, Award, DollarSign } from 'lucide-react';

interface HeroSectionProps { isSupplierView?: boolean; }

export default function HeroSection({ isSupplierView = false }: HeroSectionProps) { const [animatedValue, setAnimatedValue] = useState(0); const [showConfetti, setShowConfetti] = useState(false);

useEffect(() => { const timer = setTimeout(() => { let current = 0; const target = 15; const increment = target / 30;
...
if (isSupplierView) { return ( <div className="relative bg-gradient-to-br from-[#0071ce] via-blue-700 to-[#367c2b] text-white overflow-hidden"> {/* Supplier Banner */} <div className="absolute top-0 left-0 right-0 bg-[#ffc220] text-[#0071ce] py-2 z-10"> <div className="max-w-7xl mx-auto px-4 text-center"> <div className="flex items-center justify-center space-x-3 text-sm font-bold"> <Factory className="h-4 w-4" /> <span>🏭 WALMART SUPPLIER PORTAL</span> <span>•</span> <span>OFFICIALLY ENDORSED BY WALMART'S SUSTAINABILITY INNOVATION LAB</span> <Factory className="h-4 w-4" /> </div> </div> </div>
...
// You can add consumer-facing content here if needed return null; }